...
- 每次遍历价格，i 将会变化，同时也要注意 k 的变化 k 的变化只存在于购买的时候 这里循环和 k 的关系还需要理解 TODO:
...
- k = 2 类似 k = 1，但是每天变成了有四个变量，因为需要确认当天是第几次操作，即第一次有两个变量，第二次有两个变量
...
- k = 任意数 假定 prices 的长度是 n 如果 k >= n / 2 这种情况和 k = +INF 相同，另一种情况则是每次循环内部循环 k 次 TODO: 需要再理解
