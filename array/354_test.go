...
/*
	...
	设 f[j] 表示 h 的前 i 个元素可以组成长度为 j 的最长严格递增子序列的末尾元素的最小值。f 数值是严格单调递增的

	状态转移的时候，考虑当前元素 hi，这里就类似 300 了
		如果 hi 大于 f 中的最大值，则可以直接放在 f 的最后，形成更长的子序列
		否则，找到 f 中比 hi 严格小的最大元素 fj0 fj0 < hi <= fj0+1 则 hi 可以接在 fj0 之后，形成长度为 j0+1 的严格递增子序列，因此需要对
		fj0+1 更新，fj0+1 = hi，可以使用二分查找来找到满足的 j0
	这里的 f 的更新可以参考 300
*/
